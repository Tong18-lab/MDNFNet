...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if 'LTCC_ReID' in img_path:
            ...
            parsing_result_path = os.path.join('', '/'.join(img_path.split('/')[:6]), 'processed',
        ...
        # 转换为可修改的 NumPy 数组副本
        img_b = np.array(img_copy, dtype=np.uint8).copy()
        img_b = img_b.transpose(2, 0, 1)  # 转置维度
        # ✅ 添加衣服区域遮罩
        clothes_mask = (parsing_result_copy == 2) | (parsing_result_copy == 3) | (parsing_result_copy == 4) | \
                       (parsing_result_copy == 5) | (parsing_result_copy == 6) | (parsing_result_copy == 7) | \
                       (parsing_result_copy == 10) | (parsing_result_copy == 11)

        # ✅ 用彩色噪声替代衣服区域
        random_noise = np.random.randint(0, 256, size=img_b.shape, dtype=np.uint8)
        img_b = np.where(clothes_mask, random_noise, img_b)  # 在衣服区域注入噪声
