...
from models.LGFI import LGFI_Module
...
def train_model(config, epoch, model, attention, gap_classifier, gap_classifier_h, gap_classifier_b, criterion_cla, criterion_pair, optimizer, trainloader):
    ...
    for batch_idx, (parsing_results, imgs, imgs_b, pids, clothes_ids) in enumerate(trainloader):
        ...
        features, features_bb = model(torch.cat((imgs, imgs_b), dim=0)).split(imgs.size(0), dim=0)
        lgfi = LGFI_Module(in_channels=2048).cuda()
        features_b = lgfi(features_bb)

        # features = {Tensor: (32,2048,24,18)},imgs = {Tensor: (32,3,384,288)}
        ...
        loss = 0.01*part_loss + 0.01*vc_loss + pair_loss_f + pair_loss_h + pair_loss_b + cla_loss_f + cla_loss_h + cla_loss_b
